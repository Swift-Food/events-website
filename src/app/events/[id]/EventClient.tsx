...
export default function EventClient({ initialEvent, eventId }: EventClientProps) {
 ...
 {userRole && (
 <div className={`sm:hidden ${userRole ==="scanner"?"bg-blue-500/10": userRole ==="admin"?"bg-purple-500/10":"bg-amber-500/10"}`}>
 <div className="px-6 py-3 flex items-center justify-between gap-4">
 <span className="text-sm text-neutral-300">
 {userRole ==="scanner"?"You can scan tickets for this event.": userRole ==="owner"?"You own this event.":"You have manage access for this event."}
 </span>
 {userRole ==="scanner"? (
 <Link
 ...
 userRole ==="owner"?"bg-amber-500 hover:bg-amber-600":"bg-purple-500 hover:bg-purple-600"}`}
 >
 Manage
 ...
 {userRole && (
 <div className={`hidden sm:flex mb-6 items-center justify-between gap-4 rounded-lg px-4 py-3 ${userRole ==="scanner"?"bg-blue-500/10": userRole ==="admin"?"bg-purple-500/10":"bg-amber-500/10"}`}>
 <div className="flex items-center gap-3">
 <span
 className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${
 userRole ==="owner"?"bg-amber-500/30 text-amber-400": userRole ==="admin"?"bg-purple-500/30 text-purple-400":"bg-blue-500/30 text-blue-400"}`}
 >
 ...
 <div
 className="flex h-full items-center justify-center"style={{ backgroundColor: event.eventColor ||"#3b82f6"}}
 >
 ...
 target="_blank"rel="noopener noreferrer"className="flex items-center gap-2 text-sm text-primary hover:underline">
 ...
 className="h-40 w-full !rounded-none"placeId={event.address.placeId}
 />
 ) : (
 <div className="h-40 w-full bg-card-secondary-background flex flex-col items-center justify-center gap-2">
 <MapPin className="h-5 w-5 text-muted-foreground"/>
 ...
 <a
 href={formatExternalUrl(event.virtualMeetingUrl)}
 target="_blank"rel="noopener noreferrer"className="flex items-center gap-2 text-sm text-primary hover:underline mb-2">
 <ExternalLink className="h-4 w-4"/>
 Join Virtual Meeting
 </a>
 ) : (
 ...
 <Link
 href="/my-tickets"className="shrink-0 rounded-lg bg-blue-500/20 px-4 py-2 text-sm font-medium text-blue-300 transition-colors hover:bg-blue-500/30">
 View My Tickets
 </Link>
 </div>
 </div>
 ...
 isOwnedTicket
 ? isActiveTicket
 ?"bg-green-500/10 cursor-default":"bg-yellow-500/10 cursor-default": isInvitedTicket
 ?"bg-emerald-500/10 cursor-pointer ring-2 ring-emerald-500/30": isDisabledByInvitation
 ?"bg-card-secondary-background opacity-40 cursor-not-allowed": isDisabled
 ?"bg-card-secondary-background opacity-50 cursor-not-allowed": isSelected
 ?"bg-primary/10 cursor-pointer":"bg-card-secondary-background cursor-pointer"}`}
 >
 ...
 ?"Free":`£${Number(ticket.price).toFixed(2)}`}
 </p>
 ...
 {event.description ? (
 <div
 className="tiptap-editor tiptap-view-mode"dangerouslySetInnerHTML={{ __html: event.description }}
 />
 ) : (
 ...
 placeholder="Describe the issue with this event..."className="mb-4 w-full rounded-lg bg-card-secondary-background px-4 py-3 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 min-h-[150px] resize-y"disabled={isReporting}
 />
