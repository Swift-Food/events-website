...
export default function NotificationItem({
    ...
    switch (action) {
      ...
      case NotificationActionType.ACCEPT_COLLAB:
        return (
          <button
            ...
            className={`${baseClasses} bg-accent text-white hover:bg-accent/80`}
          >
            Accept
          </button>
        );
      ...
      case NotificationActionType.VIEW_PROFILE:
        return (
          <button
            onClick={() => handleAction(notification, action)}
            className={`${baseClasses} text-accent hover:bg-accent/10`}
          >
            View Profile
          </button>
        );
  ...
  return (
    ...
    >
      {/* Unread indicator */}
      {isUnread && (
        <div className="absolute left-1.5 top-1/2 -translate-y-1/2 h-2 w-2 rounded-full bg-accent" />
      )}
